...
```json
{
  "mcpServers": {
    "time": {
      "command": "docker",
      "args": ["run", "-i", "--rm", "-e", "${DOCKER_REGISTRY_TAG}_TIMEZONE", "mcp/time"]
    }
  }
}
```
...
By default, the server automatically detects your system's timezone. You can override this by adding the argument `--${DOCKER_REGISTRY_TAG}-timezone` to the `args` list in the configuration.

Example:
```json
{
  "command": "python",
  "args": ["-m", "mcp_server_time", "--${DOCKER_REGISTRY_TAG}-timezone=America/New_York"]
}
```
